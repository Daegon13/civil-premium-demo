...
import { HERO_3D_TUNING } from "./lib/heroTuning";
...
export function CivilPremiumHeroModel() {
  ...
  const tunedBaseCamera = useMemo<[number, number, number]>(() => {
    ...
    const direction = camera.sub(target).multiplyScalar(HERO_3D_TUNING.cameraDistanceFactor);
  ...
  return (
    <div
      ...
      style={{
        ...
        maxHeight: `${HERO_3D_TUNING.stageMaxHeightRem}rem`,
      }}
      onPointerMove={(event) => {
